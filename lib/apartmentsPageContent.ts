...
export const apartmentsPageContent: Record<Language, ApartmentsPageCopy> = {
  ru: {
    ...
    intro: "В каталоге RentPlaceMD собраны квартиры и апартаменты для посуточной и краткосрочной аренды в Кишинёве. Если нужно снять квартиру на сутки или на более продолжительный срок, сравните актуальные цены, реальные фотографии, адрес, планировку и указанную вместимость каждого варианта. На странице конкретной квартиры можно посмотреть доступные даты, условия проживания и подробные характеристики. После выбора свяжитесь с администратором RentPlaceMD по телефону или через удобный мессенджер, чтобы уточнить свободные даты и отправить запрос на бронирование напрямую.",
